...
/**
 * Brigadier argument type for selecting {@link Celestial}s. Mirrors vanilla's
 * entity selector syntax but uses {@code @c} and only supports the {@code id=}
 * option, e.g. {@code @c[id=space_engine_s:earth]}.
 *
 * <p>Ported from {@code shipwrights.genesis.commands.CelestialArgument}; no
 * Forge-specific APIs were involved, so the port is mostly an import sweep.
 */
public class CelestialArgument implements ArgumentType<CelestialArgument.CelestialSelector> {
    ...
    public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> context, SuggestionsBuilder builder) {
        ...
        if (remaining.startsWith("@c[")) {
            ...
            if (context.getSource() instanceof SharedSuggestionProvider source) {
                source.registryAccess().registry(SpaceengineS.CELESTIALS_KEY)
    ...
    public static class CelestialSelector {
        ...
        List<Celestial> find(CommandSourceStack source) {
            Registry<Celestial> registry = SpaceengineS.getCelestialRegistry(source.getLevel());
