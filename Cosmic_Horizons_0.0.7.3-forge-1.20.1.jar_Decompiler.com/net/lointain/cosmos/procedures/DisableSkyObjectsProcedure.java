...
public class DisableSkyObjectsProcedure {
   ...
   public static void renderStars(int amount, int seed, float yaw, float pitch, float roll, int color, boolean constant) {
      if (starBuffer != null && amount == DisableSkyObjectsProcedure.amount && seed == DisableSkyObjectsProcedure.seed) {
         starBuffer.m_85921_();
      } else {
         DisableSkyObjectsProcedure.amount = amount;
         DisableSkyObjectsProcedure.seed = seed;
         RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, 1.0F);
         RenderSystem.setShader(GameRenderer::m_172808_);
         BufferBuilder bufferBuilder = Tesselator.m_85913_().m_85915_();
   ...
   private static boolean execute(@Nullable Event event, LevelAccessor world, Entity entity) {
      if (entity == null) {
         ...
         if (CosmosModVariables.WorldVariables.get(world).dimensional_data.m_128441_(entity.m_9236_().m_46472_().m_135782_().toString())) {
            ...
            if (dimensional_data.m_128441_("sky_objects")) {
               var7 = dimensional_data.m_128423_("sky_objects");
               boolean var15;
               if (var7 instanceof ByteTag) {
                  ByteTag _byteTag = (ByteTag)var7;
                  var15 = _byteTag.m_7063_() == 1;
               } else {
                  var15 = false;
               }
               ...
               if (!var15) {
                  if (CosmosModVariables.WorldVariables.get(world).skybox_data_map.m_128441_(entity.m_9236_().m_46472_().m_135782_().toString())) {
                     Tag var8 = CosmosModVariables.WorldVariables.get(world).skybox_data_map.m_128423_(entity.m_9236_().m_46472_().m_135782_().toString());
                     CompoundTag var16;
                     if (var8 instanceof CompoundTag) {
                        CompoundTag _compoundTag = (CompoundTag)var8;
                        var16 = _compoundTag.m_6426_();
                     } else {
                        var16 = new CompoundTag();
                     }

                     CompoundTag skybox_data = var16;
                     if (skybox_data.m_128441_("vanilla_sunlight")) {
                        var8 = skybox_data.m_128423_("vanilla_sunlight");
                        boolean var17;
                        if (var8 instanceof ByteTag) {
                           ByteTag _byteTag = (ByteTag)var8;
                           var17 = _byteTag.m_7063_() == 1;
                        } else {
                           var17 = false;
                        }
